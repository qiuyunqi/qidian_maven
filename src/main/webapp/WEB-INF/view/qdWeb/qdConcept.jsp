...
<style type="text/css">
	...
	.qdGn a{ color: #fff !important;font-size: 18px !important;}
	.qdGn a>i{visibility: visible;}
</style>
</head>
<body>
...
function createTestData(dataList){
	...
	for(var i = 0; i < data.length; i++) {
		...
		if (undefined != data[i]['infoId'] && "" != data[i]['infoId']) {
			...
			urlPath = "${ctx}/web/ai/qdConcept?id="+data[i]['id'];
		}
		if(!(Math.random()*3 >> 0)){
...
// 1分钟访问一次
setInterval(index, 6000);
$id = "${id}";
function index() {
	$.post("${ctx}/web/ai/ajaxQdConcept", {"id" : $id}, function(data){
		$("#myMetro").html("");
		createTestData(data);
		metro.init(myMetro);
	});
