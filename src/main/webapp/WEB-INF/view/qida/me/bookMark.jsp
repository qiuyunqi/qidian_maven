		...
		<div class="sidebar-right">
			...
			<div class="communit-nav">
				<ul>
					...
					<li class="communit-active"><a href="${ctx}/web/ai/collectionAll.html" title="收藏">收藏(${collectionNum})</a></li>
				</ul>
			</div>
			<!-- 答案/问题/关注/收藏 的内容 -->
			<div class="communit-lists">
				<!-- 收藏 -->
				<div class="commiInfo bookMark">
					<ul class="meansLists">
					<!-- 第一条内容(内容过多，展开全部，内容中有图片) -->
					<c:if test="${collectionNum == 0}">
					您还没有收藏问题
					</c:if>
					<c:if test="${collectionNum != 0}">
					<c:forEach items="${collectionList}" var="collection">
						<li>
							<div class="tx" onmouseover='mousove($(this),40,15,${collection.qidaQuestion.fuUser.id})'  onmouseout='mousout($(this))'>
								<a class="txImg" href="javascript:void(0)" title="头像"><img src="${empty collection.qidaQuestion.fuUser.userAvatar?'../images_qiDa/defTx.jpg':collection.qidaQuestion.fuUser.userAvatar}" alt="头像"/></a>
							</div>
							<div class="discussner">
								<div class="infoBody">
									<!-- 问题 -->
									<div class="zhidao-question">
												<c:if test="${collection.qidaQuestion.isDelete==1}">
													<s>${collection.qidaQuestion.title}</s><span class="redCol">(已删除)</span>
												</c:if>
												<c:if test="${collection.qidaQuestion.isDelete!=1}">
													<a class="meTits" href="${ctx}/web/qida/questionPay/${collection.qidaQuestion.id}.html" target="_blank">
													${collection.qidaQuestion.title}
													</a>
												</c:if>
											<a class="money"><i class="${collection.qidaQuestion.replyNum==0?'mony':'moneyAcv'}"></i>${collection.qidaQuestion.reward}</a>
											<!-- 已回答 -->
									</div><!-- 问题 -->
									<!-- 回答/浏览 -->
									<div class="zhidao-question-unit-count3">
										<ul>
											<li><span class="glyphicon glyphicon-comment"></span>${collection.qidaQuestion.replyNum}回答</li>
											<li><span class="glyphicon glyphicon-eye-open"></span>${collection.qidaQuestion.pageViews}浏览</li>
										</ul>
									</div>
									<div class="short-content">
										<div class="replayCont">${collection.qidaQuestion.detail}</div>
										<div class="replayChan"></div>
										<c:if test="${not empty collection.qidaQuestion.imgUrl}">
										<div class="short-content-img">
											<img src="${collection.qidaQuestion.imgUrl}" alt="问题图片"/>
										</div>
										</c:if>
									</div>
									<!-- 标签 -->
									<div class="zhidao-question-tags clearfix">
										<ul>
											<c:forEach items="${collection.qidaQuestion.tagQuestions}" var="tagQuestion">
											<li>
									...
									<div class="zhidao-question-unit-count2">
										<ul>
											<li><span class="glyphicon glyphicon-time"></span><fmt:formatDate value="${collection.qidaQuestion.createTime}" pattern="yyyy-MM-dd HH:mm:ss"/></li>
										</ul>
									</div>
								</div>
							</div>
			...
			<div class="page">
				<domi:pagination ctx="${ctx}" pageVolume="${pageSize}"
					url="${ctx}/web/ai/collectionAll.html"
