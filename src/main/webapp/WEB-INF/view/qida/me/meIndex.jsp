	...
	<div class="container bodyHget">
		...
		<div class="sidebar-right">
			...
			<div class="communit-nav">
				<ul>
					<li class="communit-active"><a href="${ctx}/web/ai/meindex.html" title="答案">答案(${answerNum})</a></li>
			...
			<div class="communit-lists">
				<!-- 答案 -->
				<div class="commiInfo meAns">
					<ul class="meansLists">
					<!-- 第一条内容(内容过多，展开全部，内容中有图片) -->
					<c:if test="${answerNum == 0}">
					您还没有回答过问题
					</c:if>
					<c:if test="${answerNum != 0}">
							...
							<div class="discussner">
								<div class="infoBody">
									...
									<div class="zhidao-question">
											<c:if test="${answer.qidaQuestion.isDelete==1}">
												<s>${answer.qidaQuestion.title}</s><span class="redCol">(已删除)</span>
											</c:if>
											<c:if test="${answer.qidaQuestion.isDelete!=1}">
												<c:if test="${answer.isDelete==1}">
													${answer.qidaQuestion.title}
												</c:if>
												<c:if test="${answer.isDelete!=1}">
													<a class="meTits" href="${ctx}/web/ai/answerInfo/${answer.id}.html" target="_blank">
													${answer.qidaQuestion.title}
													</a>
												</c:if>
											</c:if>
										<a class="money"><i class="${answer.qidaQuestion.replyNum==0?'mony':'moneyAcv'}"></i>${answer.qidaQuestion.reward}</a>
									</div><!-- 问题 -->
									<!-- 回答/浏览 -->
									<div class="zhidao-question-unit-count3">
										<ul>
											<li><span class="glyphicon glyphicon-comment"></span>${answer.qidaQuestion.replyNum}回答</li>
											<li><span class="glyphicon glyphicon-eye-open"></span>${answer.qidaQuestion.pageViews}浏览</li>
										</ul>
									</div>
									<!-- 回答 -->
									...
									<div class="zhidao-question-tags clearfix">
										<ul>
											<c:forEach items="${answer.qidaQuestion.tagQuestions}" var="tagQuestion">
											<li>
									...
									<div class="zhidao-question-unit-count2">
										<ul>
											<li><span class="glyphicon glyphicon-time"></span><fmt:formatDate value="${answer.createTime}" pattern="yyyy-MM-dd HH:mm:ss"/></li>
										</ul>
									</div>
								</div>
							</div>
